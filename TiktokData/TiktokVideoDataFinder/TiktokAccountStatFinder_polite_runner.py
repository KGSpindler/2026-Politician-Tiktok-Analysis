...
BETWEEN_SCROLL_MIN = 3.0
BETWEEN_SCROLL_MAX = 6.0
BETWEEN_VIDEO_FETCH_MIN = 1.5
BETWEEN_VIDEO_FETCH_MAX = 4.0
BETWEEN_PROFILE_MIN = 20.0
BETWEEN_PROFILE_MAX = 45.0
COOLDOWN_EVERY_N_PROFILES = 5
COOLDOWN_MIN_SECONDS = 180
COOLDOWN_MAX_SECONDS = 420
...
def tiktok_error_visible(driver):
    ...
    return any(marker in src for marker in error_markers)
...
def recover_profile_page(driver, profile_url):
    for attempt in range(1, MAX_RECOVERY_ATTEMPTS + 1):
        print(f"    Recovery-forsøg {attempt}/{MAX_RECOVERY_ATTEMPTS}")
        human_pause(12, 25, "før refresh")
        try:
            driver.get(profile_url)
        except Exception:
            pass
        human_pause(6, 12, "efter refresh")
        if not tiktok_error_visible(driver):
            return True
    return False
    ...
    try:
        ...
        for scroll_idx in range(MAX_SCROLLS):
            if tiktok_error_visible(driver):
                ok = recover_profile_page(driver, profile_url)
                if not ok:
            ...
            if stagnant_scrolls >= MAX_STAGNANT_SCROLLS:
                print("    Ingen nye links efter flere scrolls — forsøger recovery")
                ok = recover_profile_page(driver, profile_url)
                if not ok:
                    raise RetryLaterError("Profil gav ingen nye links efter recovery")
                stagnant_scrolls = 0
                continue
